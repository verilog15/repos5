...
# triplet_loss = torch.nn.TripletMarginLoss(margin=1.0, p=1)
triplet_loss = torch.nn.TripletMarginLoss(margin=1.2, p=1) # Test only
...
percep_triplet_loss = TripletMarginWithDistanceLoss(distance_function=lambda x, y: lpips_loss(x, y), margin=1.2)  #
