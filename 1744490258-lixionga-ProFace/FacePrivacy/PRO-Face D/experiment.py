...
from functions.denoising import arcface_ddim_diffusion_privacy_protection
...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        # Autoenc: Semantic Encoder + Conditional DDIM
    ...
    def render(self, noise, cond=None, T=None, energy_fun_conf=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
    ...
    def sample_image_alogrithm_arcface_ddim(self, x, last=True, cls_fn=None, rho_scale=1.0, stop=100,
                                            ref_path=None, device='cpu', tao=0, energy_fun='res', model_kwargs=None):
        """
        Args:
            x: 初始噪声 xT
            model: unet
        ...
        assert x.shape == (1, 3, 256, 256), "xT维度错误"
        # print(f"   5.1 algorithm begin...  rho_scale={rho_scale}")
        ...
        betas = get_beta_schedule(
            ...
            beta_start=0.0001,  # β_1 = 1^{-4},   β_1 < β_2 < ... < β_T （理解为：加噪越来越强烈）
        ...
        model = self.ema_model
        x = arcface_ddim_diffusion_privacy_protection(x, seq, model, betas,
                                   cls_fn=cls_fn, rho_scale=rho_scale, stop=stop,
                                   ref_path=ref_path, tao=tao, energy_fun=energy_fun, model_kwargs=model_kwargs)

        if last:
            x = x[0][-1]
        return x

    def sample_xt_from_x0(self, x, t, T=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        timesteps = sorted(sampler.conf.use_timesteps)
        t2i = {j: i for i, j in zip(indices, timesteps)}
        # print(t2i)
        # print(x.shape)
        # 采样 xt = a * x0 + b * noise
        t = t2i[t]
        t_batch = th.tensor([t] * x.shape[0], device=x.device)
        # print(f"t info: {t_batch}")
        noise = torch.randn_like(x)
        # th.save(noise, f'test/guide_idis/xT_epsilon{t}.pt')
        xt = sampler.q_sample(x_start=x, t=t_batch, noise=noise)
        assert x.shape == xt.shape, "xt 和 x0 的维度不相等"
        return xt

    def render_anony(self, noise, cond, t_inner, T=None, anony_fun_conf=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        # DiffAE
        assert cond is not None, "cond must not None"
        pred_img = render_anony_condition(self.conf,
                                    self.ema_model,
                                    noise,
                                    sampler=sampler,
                                    cond=cond,
                                    anony_fun_conf=anony_fun_conf,
                                    t_inner=t_inner)

        pred_img = (pred_img + 1) / 2
        return pred_img

    def render_recovery(self, pwd_xT, pwd_cond, T=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        # DiffAE
        assert pwd_cond is not None, "pwd_cond must not None"
        assert pwd_xT is not None, "pwd_xT must not None"

        pred_img = render_recovery_condition(self.conf,
                                    self.ema_model,
                                    pwd_xT=pwd_xT,
                                    sampler=sampler,
                                    pwd_cond=pwd_cond)
    ...
    def render_swap(self, noise, cond, t_inner, loss_fn_alex, T=None, swap_fun_conf=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        # DiffAE
        assert cond is not None, "cond must not None"
        pred_img = render_swap_condition(self.conf,
                                    self.ema_model,
                                    noise,
                                    sampler=sampler,
                                    cond=cond,
                                    swap_fun_conf=swap_fun_conf,
