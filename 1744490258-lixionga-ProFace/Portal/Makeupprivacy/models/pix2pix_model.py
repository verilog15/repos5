import torch
import Makeupprivacy.models.networks as networks
# import models.LADN_master as LADN_master

import Makeupprivacy.util.util as util
from Makeupprivacy.models.networks.architecture import VGG19
import torch.nn.functional as F
import Makeupprivacy.Pretrained_FR_Models.irse as irse
import Makeupprivacy.Pretrained_FR_Models.facenet as facenet
import Makeupprivacy.Pretrained_FR_Models.ir152 as ir152
import cv2
import os
    ...
    def __init__(self, opt):
        ...
        for model_name in self.train_model_name_list + self.val_model_name_list:
            ...
            if model_name == 'ir152':
                ...
                self.fr_model.load_state_dict(torch.load('/home/chenyidou/x_test/web/Makeupprivacy/checkpoints/ir152.pth', map_location=torch.device('cpu')))
                # print("1")
            if model_name == 'irse50':
                self.models_info[model_name][0].append((112, 112))
                self.fr_model = irse.Backbone(50, 0.6, 'ir_se')
                self.fr_model.load_state_dict(torch.load('/home/chenyidou/x_test/web/Makeupprivacy/checkpoints/irse50.pth', map_location=torch.device('cpu')))
            if model_name == 'mobile_face':
                self.models_info[model_name][0].append((112, 112))
                self.fr_model = irse.MobileFaceNet(512)
                self.fr_model.load_state_dict(torch.load('/home/chenyidou/x_test/web/Makeupprivacy/checkpoints/mobile_face.pth', map_location=torch.device('cpu')))
            if model_name == 'facenet':
                ...
                self.fr_model.load_state_dict(torch.load('/home/chenyidou/x_test/web/Makeupprivacy/checkpoints/facenet.pth', map_location=torch.device('cpu')))
