...
class Partial_conv3(nn.Module): #PConv

    def __init__(self, dim, n_div,kernel_size,stride,padding): # 根据不同输入选择不同的前向传播方式
        super().__init__()
        self.dim_conv3 = dim // n_div
        if (self.dim_conv3 ==0):
            self.dim_conv3 = dim
        self.dim_untouched = dim - self.dim_conv3
        self.partial_conv3 = nn.Conv2d(self.dim_conv3, self.dim_conv3,kernel_size,stride,padding,bias=False)
    ...
    #     x[:, :self.dim_conv3, :, :] = self.partial_conv3(x[:, :self.dim_conv3, :, :])#对输入张量的前一部分进行卷积操作

    #     return x

    def forward(self, x):
        # for training/inference
        x1, x2 = torch.split(x, [self.dim_conv3, self.dim_untouched], dim=1)  # 将输入张量按照维度1分割成两个张量
        x1 = self.partial_conv3(x1) # 对前一部分进行卷积操作
        x = torch.cat((x1, x2), 1) # 将两个张量拼接在一起
...
class UNet(nn.Module):
    def __init__(self):
        ...
        for i in range(len(self.Encoder_channel)-1):
            groups = self.Encoder_channel[i] // 4
            if groups == 0:
                groups = 1
            self.Encoder.append(nn.Sequential(*[
                # Partial_conv3(self.Encoder_channel[i],4,3,1,1),
                nn.Conv2d(self.Encoder_channel[i], self.Encoder_channel[i], kernel_size=4, stride=2, padding=1,groups=groups),
                # nn.Conv2d(self.Encoder_channel[i], self.Encoder_channel[i+1], kernel_size=1,stride=2),
        ...
        for i in range(len(self.Decoder_inchannel)):   
            # groups = self.Decoder_inchannel[i] // 4
            # if groups == 0:
            #     groups = 1
            self.Decoder.append(nn.Sequential(*[
                # Partial_conv3(self.Decoder_inchannel[i],4,3,1,1),
                nn.Conv2d(self.Decoder_inchannel[i], self.Decoder_inchannel[i], kernel_size=3, stride=1, padding=1,groups=groups),
                nn.Conv2d(self.Decoder_inchannel[i], self.Decoder_outchannel[i], kernel_size=1),
                # nn.Conv2d(self.Decoder_inchannel[i], self.Decoder_outchannel[i],kernel_size=3,stride=1,padding=1)
            ]))
...
class AdaINBlock(nn.Module):
    def __init__(self,in_channel,out_channel):
        ...
        self.conv1 = nn.Conv2d(in_channel,in_channel,kernel_size=3,padding=1,groups=in_channel//4)
        self.conv2 = nn.Conv2d(in_channel,in_channel,kernel_size=3,padding=1,groups=in_channel//4)
