...
def target_get_hull(landmark,img1,setting):
...
def get_local_hull(landmark, img):
    mask_set = []
    
    eye = [landmark[36:42], landmark[17:22], landmark[42:48], landmark[22:27]]
    nose = [landmark[27:36]]
    mouth = [landmark[48:68]]
    
    local_regions = [eye, nose, mouth]

    for items in local_regions:
        mask = np.zeros_like(img[:,:,0])
        
        for item in items:
            cv2.fillConvexPoly(mask, cv2.convexHull(item), 255.)
            
        kernel_0 = (5,5)
        mask = cv2.dilate(mask, kernel_0, iterations=10)
        mask_set.append(mask)
    
    mask_set = np.stack(mask_set)

    return mask_set


def random_get_local_hull(landmark, img):
    num = random.choice([1, 2, 3]) # eye nose mouth eyebrow
    hull_types = random.sample([0, 1, 2], num) # eye nose mouth eyebrow
    
    mask = np.zeros_like(img[:,:,0])
    
    for type in hull_types:
        if type == 0:
            eye = [landmark[36:42], landmark[42:48]]
            for item in eye:
                cv2.fillConvexPoly(mask, cv2.convexHull(item), 255.)
                
            eyebrow = [landmark[17:22], landmark[22:27]]
            for item in eyebrow:
                cv2.fillConvexPoly(mask, cv2.convexHull(item), 255.)
